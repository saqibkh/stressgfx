...
__global__ void readKernel(int* a, size_t size) {
...
void callReadKernel(int* d_a, int test_duration, size_t memory_size) {
    ...
    while (std::chrono::duration_cast<std::chrono::seconds>(end_time - start_time).count() < test_duration) {
        ...
        readKernel<<<blocksPerGrid, threadsPerBlock>>>(d_a, num_elements);
        hipEventRecord(stop);
    ...
    std::cout << "Average bandwidth for Read: " << average_bandwidth << " GB/s" << std::endl;
}
