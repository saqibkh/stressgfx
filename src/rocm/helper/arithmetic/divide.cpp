...
__global__ void checkMiscompareDivideKernel(const int* a, const int* b, const int* c, int* miscompareIndex, int* actualValue, int* expectedValue, size_t N) {
...
int callDivideKernel(int* d_a, int* d_b, int* d_c, size_t memory_size, int testDuration) {

    std::cout << "============= TEST RESULTS FOR DIVIDE =============" << std::endl;
    ...
    while (std::chrono::duration<double>(end - start).count() < testDuration){
        hipLaunchKernelGGL(divideKernel, dim3(blocksPerGrid), dim3(threadsPerBlock), 0, 0, d_a, d_b, d_c, num_elements);
        hipDeviceSynchronize();

	if(CHECK_RESULT == true){
            // Launch kernel to compare results on the GPU
	    hipLaunchKernelGGL(checkMiscompareDivideKernel, 
			    dim3(blocksPerGrid), 
    ...
    std::cout << "Bandwidth (DIVIDE): " << bandwidth << " GB/s\n" << std::endl;
    return l_fail;
}
