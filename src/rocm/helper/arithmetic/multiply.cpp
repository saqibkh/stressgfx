...
__global__ void multiplyKernel(int* a, int* b, int* c, size_t size) {
...
__global__ void checkMiscompareMultiplyKernel(const int* a, const int* b, const int* c, int* miscompareIndex, int* actualValue, int* expectedValue, size_t N) {
...
int callMultiplyKernel(int* d_a, int* d_b, int* d_c, size_t memory_size, int testDuration) {

    std::cout << "============= TEST RESULTS FOR MULTIPLY =============" << std::endl;
    ...
    while (std::chrono::duration<double>(end - start).count() < testDuration){
        hipLaunchKernelGGL(multiplyKernel, dim3(blocksPerGrid), dim3(threadsPerBlock), 0, 0, d_a, d_b, d_c, num_elements);
        hipDeviceSynchronize();

	if(CHECK_RESULT == true){
            // Launch kernel to compare results on the GPU
	    hipLaunchKernelGGL(checkMiscompareMultiplyKernel, 
			    dim3(blocksPerGrid), 
    ...
    std::cout << "Bandwidth (MULTIPLY): " << bandwidth << " GB/s\n" << std::endl;
    return l_fail;
}
