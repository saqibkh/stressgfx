...
__global__ void multipleKernel(float* a, float* b, float* c, size_t size) {
...
void callMultiplyKernel(int test_duration, size_t memory_size) {
    ...
    */
    ...
    while (std::chrono::duration_cast<std::chrono::seconds>(end_time - start_time).count() < test_duration) {
        ...
        multipleKernel<<<blocksPerGrid, threadsPerBlock>>>(d_a, d_b, d_c, num_elements);
        hipEventRecord(stop);
    ...
    std::cout << "Average bandwidth for Multiple: " << average_bandwidth << " GB/s" << std::endl;
